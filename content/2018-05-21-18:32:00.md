...
Not commited yet
--
This documents the upcoming configuration change that will happen in OpenSMTPD.

The code is not yet commited,
people willing to test should join the IRC channel #OpenSMTPD at freenode,
wait for the code to land in the OpenBSD tree soon,
or wait for us to generate new snapshots published on the OpenSMTPD mailing list.

The new grammar is quite stable now but minor changes may still happen.
