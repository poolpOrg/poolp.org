...
This is done by adding the `dkim_signing` filter to the `filters` key configuration in `/etc/rspamd/options.inc`:
```
...
but **in this setup** OpenSMTPD authenticates against simple usernames. Also make sure the `dkim_signing.conf` is
