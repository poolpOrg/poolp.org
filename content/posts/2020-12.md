...
The **[6.8.0p1 release was published today](https://www.mail-archive.com/misc@opensmtpd.org/msg05188.html)** and **contains the fixes** for the issues mentionned below.
...
I'd like to thank again that user who wished to remain anonymous,
the money from his sponsored development helped me replace my laptop with a new MacBook Air (and the Apple M1 SoC) which is lovely,
and the result from this sponsored development **benefits all users of OpenSMTPD portable**.
...
another user reported a **memory leak** which caused me to go on a big hunt.
...
I didn't have an immediate intuition where the leak could come from as could happen with other processes.
...
It turns out that a `regfree()` call was missing causing a `regex_t` to leak for each regex lookup.
...
# Work on the OpenSMTPD post-release
