...
---
    ...
    - elle n'est pas très pratique aujourd'hui
    - beaucoup de gens se passent du greylisting ou trouvent des contournements
    - le SPF-aware greylisting rends le greylisting utilisable à nouveau
...
Concrètement,
un noeud SMTP a **deux** états finaux pour un message :
soit le message **est distribué** et le noeud destination le détient,
soit il **n'est PAS distribué** et le noeud destination ne le détient pas.
Dans le second cas,
le protocole SMTP exige que le dernier noeud en charge notifie l'émetteur qu'une erreur est survenue.
...
mais qui **pourraient** tout aussi bien ne plus se produire si **quelque chose** était corrigé sur le noeud de destination.
...
Il ne sont _généralement_ vus par les utilisateurs que lorsque les tentatives finissent par réussir ou qu'elles échouent définitivement après que leurs temps de vie ait expiré.
...
Il existe un mécanisme dans le protocole SMTP qui permets à un MX destinataire de demander à un MX émetteur de retransmettre un message.
...
**Honorer les retransmissions ralenti considérablement l'émission pour un résultat incertain**.
...
Il permets par contre d'éliminer la plus grosse partie du spam :
...
Est-ce que cela mets en défaut le greylisting ?
Pas vraiment.
...
Pendant un moment,
j'ai utilisé une approche naive consistant à whitelister le `/24` de n'importe quel MX de Big Mailer Corps qui se retrouvait greylisté.
...
un outil qui permets de faire une énumération dans l'enregistrement SPF d'un domaine et extraire un maximum d'adresse IP possible.
Il y a un [article en anglais à propos de spfwalk](/posts/2018-01-08/spfwalk/) sur ce blog si ça vous intéresse.
L'outil avait pour vocation de remplacer mon approche naive,
il n'est en loin parfait,
c'est du _best effort_.

La façon dont fonctionne SPF permets de vérifier facilement si une adresse IP est autorisée,
mais ne permets pas d'extraire facilement une liste d'adresse pour une vérification ultérieure.
Par exemple,
...
il n'y a _aucune_ adresse IP a extraire pour mettre dans une whitelist.
Donc…
...
Je vais juste clarifier avant tout que **je ne prétend pas avoir inventé cette méthode**,
...
Commes les Big Mailer Corps requièrent que les émetteurs fournissent un enregistrement SPF pour ne pas dégrader leurs réputations,
...
englobant l'ensemble des Big Mailer Corps pour des raisons évidentes (difficile d'impose SPF à d'autres sans se l'imposer à soi-même).
Pour ceux-ci,
...
À ce stade,
le filtre peut faire une recherche SPF pour le domaine de l'émetteur d'envelope,
`poolp.org`.
...
Cette résolution SPF en live permets de garantir qu'il n'y a pas besoin de whitelister au préalable les adresses IP.
À la place il est possible de whitelister des domaines indépendemment des adresses IP avec lesquelles ils nous contacterons.
Cette façon de fonctionner est de plus compatible avec les domaines qui ne fournissent pas d'enregistrements SPF,
...
je ne vois aucune différences de traitement entre Gmail et le petit domaine du coin.
